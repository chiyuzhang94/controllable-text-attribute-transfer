...
def eval_iters(ae_model, dis_model):
    ...
    eval_data_loader.create_batches(eval_file_list, eval_label_list,device, if_shuffle=False)
    ...
    for it in range(eval_data_loader.num_batch):
        ...
        # Define target label
        target = get_cuda(torch.tensor([[1.0]], dtype=torch.float),device)
        if tensor_labels[0].item() > 0.5:
            target = get_cuda(torch.tensor([[0.0]], dtype=torch.float),device)
...
if __name__ == '__main__':
    ...
    if torch.cuda.is_available():
        if torch.cuda.device_count() == 1:
            ...
            dis_model = dis_model.to(device)
            dis_model = nn.DataParallel(dis_model, device_ids=device_ids)
    else:
        ae_model = ae_model
        dis_model = dis_model
